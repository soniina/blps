...
class OrderServiceImpl(
    ...
    override fun process(request: OrderRequest, username: String, items: List<CartItem>): Order {
        ...
        try {
            ...
            jiraTicketKey = warehouseJcaService.reserveOrder(order.id!!, items)

            cartService.clearCart(username)
            return order
        } catch (e: Exception) {
            jiraTicketKey?.let { key ->
                println("ROLLBACK: Deleting Jira ticket $key due to error: ${e.message}")
                try {
                    warehouseJcaService.cancelReservation(key)
                } catch (rollbackError: Exception) {
                    println("CRITICAL: Manual rollback failed for Jira ticket $key: ${rollbackError.message}")
                }
            }
            throw e
        }
